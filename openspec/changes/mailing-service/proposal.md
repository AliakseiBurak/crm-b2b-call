...
Рассылки (`campaigns`) создаются и запускаются, но реальной отправки писем нет. Операторам нужен сервис, который по-настоящему рассылает письма по SMTP (фоновая команда опрашивает БД) и реагирует на изменения статуса рассылки. Без этого запуск рассылки — только установка флага/статуса.
...
- `campaigns`: поведение отправки писем через фоновую команду и счётчик отправленных писем.
