...
### Decision: Домашняя страница перенаправляет гостей на `/login`
`HomeController::index` проверяет `$this->getUser()`; при `null` вместо
рендера `home/index.html.twig` выполняется редирект на `app_login`.
Аутентифицированные пользователи продолжают видеть домашнюю страницу
со статистикой. Это упрощает онбординг — гость сразу попадает на форму
входа.

### Decision: Установка пароля через POST /setup-password на странице входа
На странице `/login` добавлен чекбокс «Новый пользователь»; JS-скрипт
переключает видимость двух форм: входа и установки пароля. Форма
установки пароля отправляет `POST /setup-password` с полями `email`,
`new_password`, `confirm_password`, `_csrf_token`. Контроллер
`SecurityController::setupPassword` находит пользователя через
`UserRepository::findOneByEmailWithNoPassword`, проверяет длину (≥8),
совпадение, хеширует через `UserPasswordHasherInterface` и сохраняет.
При ошибке — flash-сообщение и редирект на `/login`. Альтернатива —
отдельная страница `/setup-password` — отклонена: чекбокс на странице
входа проще и не требует дополнительной навигации.
