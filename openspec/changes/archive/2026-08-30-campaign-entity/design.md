...
**Решение**: `status` (enum: `draft`/`ready`/`launched`/`failed`/`archived`, default `draft`). `draft` — черновик, редактируется; `ready` — готова к запуску, можно добавлять адресатов; `launched` — запущена (отправка будущим сервисом MailingService); `failed` — технический статус, устанавливаемый сервисом MailingService при ошибке отправки (пользователь не может выставить вручную; сбрасывается в `ready` для повторного запуска); `archived` — архивная (в списке всегда внизу, greyout). `launchedAt` фиксируется при переходе в `launched`; `failedAt` фиксируется при переходе в `failed`.
**Почему**: полный жизненный цикл рассылки; `failed` нужен для обработки ошибок отправки (устанавливается автоматически); `archived` — для хранения завершённых кампаний.
...
- **Решено**: добавлен отдельный `status` (enum: `draft`/`ready`/`launched`/`failed`/`archived`). `launched_at` сохраняется как временная метка запуска; `status` — источник истины для состояния рассылки (по умолчанию `draft`, `ready` — можно добавлять получателей, при запуске → `launched`, `failed` — технический статус от MailingService, `archived` — завершённая кампания). Статус `failed` недоступен для выбора в форме; пользователь может только сбросить его в `ready`.
